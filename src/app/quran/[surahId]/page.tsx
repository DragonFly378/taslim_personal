    ...
    <>
        ...
        <CardHeader className="bg-gradient-to-br from-primary/10 via-background to-secondary/10 p-4 sm:p-6 md:p-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
            <div className="flex-1 space-y-3 sm:space-y-4">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm">
                <span className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-primary/20 to-primary/10 text-primary rounded-full font-semibold border border-primary/20 shadow-sm">
                  <span className={`w-2 h-2 rounded-full ${surah.tempatTurun === 'Mekah' ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500 animate-pulse'} shadow-md`} />
                  {surah.tempatTurun}
                </span>
            ...
            <div className="text-right md:text-center self-start md:self-center">
              <h3 className="font-arabic text-4xl sm:text-5xl md:text-6xl lg:text-7xl text-primary dark:text-primary-300 leading-normal drop-shadow-lg" dir="rtl" lang="ar">
                {surah.nama}
              </h3>
            </div>
          </div>
      ...
      {surahNumber !== 9 && surahNumber !== 1 && (
        <div className="text-center mb-8 sm:mb-10 p-6 sm:p-8 md:p-10 bg-gradient-to-br from-primary/10 via-secondary/5 to-background rounded-3xl border-2 border-primary/20 shadow-lg relative overflow-hidden">
          ...
          <p className="font-arabic text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-primary dark:text-primary-200 leading-normal py-2 sm:py-3 md:py-4 relative z-10 drop-shadow-sm" dir="rtl" lang="ar">
      ...
      <div className="space-y-4 sm:space-y-6">
        {ayahs.map((ayah, index) => (
          <Card
            ...
            className="card-glow border-2 overflow-hidden hover:shadow-2xl hover:border-primary/30 transition-all duration-300 scroll-mt-24 bg-gradient-to-br from-card to-card/50"
          >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center mt-10 sm:mt-12 gap-3 sm:gap-4">
        {surah.suratSebelumnya ? (
          <Button asChild variant="outline" size="lg" className="flex-1 sm:flex-none border-2 hover:bg-gradient-to-r hover:from-primary/10 hover:to-transparent hover:border-primary/40 hover:shadow-lg transition-all duration-300 active:scale-95 group">
            <Link href={`/quran/${surah.suratSebelumnya.nomor}`} className="flex items-center justify-start">
              <ChevronLeft className="mr-2 sm:mr-3 h-5 w-5 group-hover:-translate-x-1 transition-transform duration-300" />
              <div className="text-left">
                <div className="text-xs sm:text-sm text-muted-foreground font-medium">Previous</div>
                <div className="font-bold text-sm sm:text-base text-primary">{surah.suratSebelumnya.namaLatin}</div>
              </div>
            </Link>
          </Button>
        ) : (
        ...
        <Button asChild variant="ghost" className="hover:bg-primary/10 hover:shadow-md transition-all duration-300 active:scale-95 border border-transparent hover:border-primary/20">
          <Link href="/quran" className="flex items-center gap-2">
            <span className="text-sm sm:text-base font-medium">📚 All Surahs</span>
          </Link>
        </Button>

        {surah.suratSelanjutnya ? (
          <Button asChild variant="outline" size="lg" className="flex-1 sm:flex-none border-2 hover:bg-gradient-to-l hover:from-secondary/10 hover:to-transparent hover:border-secondary/40 hover:shadow-lg transition-all duration-300 active:scale-95 group">
            <Link href={`/quran/${surah.suratSelanjutnya.nomor}`} className="flex items-center justify-end">
              <div className="text-right">
                <div className="text-xs sm:text-sm text-muted-foreground font-medium">Next</div>
                <div className="font-bold text-sm sm:text-base text-secondary">{surah.suratSelanjutnya.namaLatin}</div>
              </div>
